...
createFileIfNotExist(
    componentFilePath,
    generateComponentContent(elementName),
    `${elementName}.pug`
);
createFileIfNotExist(
    scssFilePath,
    generateScssContent(elementName, elementType),
    `${elementName}.scss`
);
...
function generateScssContent(name, elementType) {
    const isFragment = elementType === "fragment";
    if (isFragment) {
...
function appendScssImport(elementImportScssFilePath, elementName) {
    ...
    if (fs.existsSync(elementImportScssFilePath)) {
        ...
        if (!fileContent.includes(importStatement)) {
            ...
            console.log(
                `❗ Импорт для ${elementName}.scss уже есть в ${elementType}s/_index.scss.`
            );
        }
    } else {
